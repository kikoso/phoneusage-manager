...
public class PhoneUsageManager {
	...
	public List<Call> retrieveCallsInformation() {
		...
		if (cursor.moveToFirst()) {
			do {
				Call call = new Call();
				call.setCountryIso(cursor.getString(cursor.getColumnIndex("countryiso")));
				...
				call.setNewCall(Utils.convertToBoolean(cursor.getString(cursor.getColumnIndex("new"))));
				call.setNumber(cursor.getString(cursor.getColumnIndex("number")));
				call.setRead(Utils.convertToBoolean(cursor.getString(cursor.getColumnIndex("is_read"))));
				...
				call.setFormattedNumber(cursor.getString(cursor.getColumnIndex("formatted_number")));
				call.setNumberType(cursor.getString(cursor.getColumnIndex("numbertype")));
				call.setDurationType(cursor.getString(cursor.getColumnIndex("duration_type")));
				call.setDuration(cursor.getString(cursor.getColumnIndex("presentation")));
				call.setNumberLabel(cursor.getString(cursor.getColumnIndex("numberlabel")));
				call.setGeocodedLocation(cursor.getString(cursor.getColumnIndex("geocoded_location")));
				call.setLookupUri(cursor.getString(cursor.getColumnIndex("lookup_uri")));
				call.setName(cursor.getString(cursor.getColumnIndex("name")));
				call.setMatchedNumber(cursor.getString(cursor.getColumnIndex("matched_number")));
				call.setSubscriptionId(cursor.getString(cursor.getColumnIndex("subscription_id")));
				call.setPhotoId(cursor.getString(cursor.getColumnIndex("photo_id")));
				call.setNormalizedNumber(cursor.getString(cursor.getColumnIndex("normalized_number")));
				call.setId(cursor.getString(cursor.getColumnIndex("_id")));
				call.setSubscriptionComponentName(cursor.getString(cursor.getColumnIndex("subscription_component_name")));
				callList.add(call);
			} while (cursor.moveToNext());
		}
		return callList;
